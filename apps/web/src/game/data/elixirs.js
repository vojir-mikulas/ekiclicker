...
export const ELIXIRS_CFG = { unlockLevel: 1500 };

export const ELIXIR_IDENTITY = { dmg: 1, gold: 1, weapon: 1, click: 1, critChance: 0 };

/* Cena škáluje s postupem (nejvyšší úroveň), ať zůstane gold-sink i v lategame.
   Spotřebka → každý kus stojí stejně (costAt ignoruje index). */
export function elixirCost(id, level) {
  const def = ELIXIRS[id];
  if (!def) return Infinity;
  return Math.ceil(def.baseCost * Math.max(1, level || 1));
}
